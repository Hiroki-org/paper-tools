...
describe("parsePositiveInt", () => {
    ...
    it("should throw an error for non-decimal numeric notation", () => {
        for (const value of ["0x10", "0b101", "0o10", "1e2"]) {
            expect(() => parsePositiveInt(value)).toThrow(`正の整数を指定してください: ${value}`);
        }
    });
});

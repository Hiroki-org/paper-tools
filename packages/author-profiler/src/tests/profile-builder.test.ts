...
describe("profile-builder helpers", () => {
	...
	it("buildTopicTimelineFromPapers creates per-year topic distributions", () => {
		...
		] as unknown as import("../services/profile-builder.js").S2Paper[]);
	...
	it("buildTopicTimelineFromPapers handles edge cases", () => {
		// empty
		expect(buildTopicTimelineFromPapers([])).toEqual([]);

		// missing year
		const noYear = buildTopicTimelineFromPapers([
			{
				paperId: "p1",
				title: "A",
				fieldsOfStudy: ["ML"],
		...
		expect(noYear).toEqual([]);

		// missing fieldsOfStudy
		const noFields = buildTopicTimelineFromPapers([
			{
				paperId: "p2",
				title: "B",
				year: 2022,
		...
		expect(noFields).toEqual([]);

		// non-array fieldsOfStudy
		const nonArrayFields = buildTopicTimelineFromPapers([
			{
				paperId: "p3",
				title: "C",
				year: 2022,
		...
		expect(nonArrayFields).toEqual([]);

		// empty fieldsOfStudy
		const emptyFields = buildTopicTimelineFromPapers([
			{
				paperId: "p4",
				title: "D",
				year: 2022,

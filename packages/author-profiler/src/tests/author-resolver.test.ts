...
describe("author-resolver", () => {
	...
	describe("looksLikeAuthorId", () => {
		it("should return true for pure numeric strings with or without whitespace", () => {
			expect(looksLikeAuthorId("123456")).toBe(true);
			expect(looksLikeAuthorId("  123  ")).toBe(true);
			expect(looksLikeAuthorId("0")).toBe(true);
			expect(looksLikeAuthorId("\n123\t")).toBe(true);
		});

		it("should return false for floats, negatives, and explicitly signed numbers", () => {
			expect(looksLikeAuthorId("123.45")).toBe(false);
			expect(looksLikeAuthorId("-123")).toBe(false);
			expect(looksLikeAuthorId("+123")).toBe(false);
			expect(looksLikeAuthorId("1e3")).toBe(false);
		});

		it("should return false for non-numeric strings and invalid formats", () => {
			...
			expect(looksLikeAuthorId("123 456")).toBe(false);
			expect(looksLikeAuthorId("123-456")).toBe(false);
			expect(looksLikeAuthorId("  ")).toBe(false);
		});
	});

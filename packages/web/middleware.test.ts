...
describe("middleware", () => {
    ...
    describe("when unauthenticated", () => {
        ...
        it("should redirect to /login for other UI routes", () => {
            ...
            expect(mockRedirect.mock.calls[0][0].href).toBe("http://localhost:3000/login");
        });
    ...
    describe("when authenticated", () => {
        describe("without database selected", () => {
            it("should redirect away from /login to /setup", () => {
                ...
                const res = middleware(req);
                expect(res).toBe("redirect-response");
                expect(mockRedirect.mock.calls[0][0].href).toBe("http://localhost:3000/setup");
            });
            ...
            it("should allow access to /api/databases and its subpaths", () => {
                const routes = ["/api/databases", "/api/databases/select", "/api/databases/123"];
                for (const route of routes) {
                    vi.resetAllMocks();
                    mockNext.mockReturnValue("next-response");
                    const req = createMockRequest(`http://localhost:3000${route}`, {
        ...
        describe("with database selected", () => {
            it("should redirect away from /login to /", () => {
                ...
                const res = middleware(req);
                expect(res).toBe("redirect-response");
                expect(mockRedirect.mock.calls[0][0].href).toBe("http://localhost:3000/");
            });

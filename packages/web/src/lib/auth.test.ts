...
describe("auth", () => {
	...
	describe("setAuthCookies", () => {
		let mockResponse: { cookies: { set: ReturnType<typeof vi.fn> } };
	...
	describe("getRefreshToken", () => {
		...
		it("should return null if cookie is not present", () => {
			...
			expect(
				getRefreshToken(cookieStore as unknown as import("./auth").CookieStore),
			).toBeNull();
		});
		...
		it("should return null when the cookie is valid but contains no token", () => {
			const sealed = sealCookieValue({
				notToken: "abc",
			} as unknown as import("./auth").CookieStore);
			const cookieStore = {
				get: vi.fn().mockReturnValue({ value: sealed }),
			};
			expect(
	...
	describe("getSelectedDatabaseId", () => {
		it("should return null if cookie is not present", () => {
			...
			expect(
				getSelectedDatabaseId(
					cookieStore as unknown as import("./auth").CookieStore,
				),
			).toBeNull();
		});
	...
	describe("isAuthenticated", () => {
		...
		it("should return false if access token is not present", () => {
			...
			expect(
				isAuthenticated(cookieStore as unknown as import("./auth").CookieStore),
			).toBe(false);
		});
	...
	describe("getUserInfo", () => {
		...
		it("should return null if cookie is not present", () => {
			...
			expect(
				getUserInfo(cookieStore as unknown as import("./auth").CookieStore),
			).toBeNull();
		});
	...
	describe("setOauthStateCookie specific options tests", () => {
		...
		it("should correctly set oauth state cookie with explicit secure false based on NODE_ENV development fallback", () => {
			...
			setOauthStateCookie(
				mockResponse,
				"state-123",
				undefined as unknown as import("./auth").RequestLike,
			);
			expect(mockResponse.cookies.set).toHaveBeenCalledWith(
				OAUTH_STATE_COOKIE,
				"state-123",
	...
	describe("getAccessToken", () => {
		...
		it("should return null if cookie is not present", () => {
			...
			expect(
				getAccessToken(cookieStore as unknown as import("./auth").CookieStore),
			).toBeNull();

...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const userName = owner.type === "user" && "name" in owner.user ? (owner.user as unknown as { name?: string | null }).name ?? undefined : undefined;

        const response = NextResponse.redirect(new URL("/setup", request.url));
        const refreshToken = (tokenResponse as unknown as { refresh_token?: string | null }).refresh_token ?? undefined;
        if (!refreshToken) {

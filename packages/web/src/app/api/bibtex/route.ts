...
import { getSearchParams, parseStringParam } from "@/lib/api/request";
...
export async function GET(request: NextRequest) {
	try {
		...
		const doi = normalizeDoi(searchParams.get("doi") ?? undefined);
		const title = parseStringParam(searchParams, "title") || undefined;
		const format = parseFormat(searchParams.get("format"));
		const keyFormat = parseKeyFormat(searchParams.get("keyFormat"));

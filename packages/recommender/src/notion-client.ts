...
export async function getDatabaseInfo(
    ...
    const databaseName = (isFullDatabase(database) ? extractPlainText(database.title) : undefined) || "(untitled database)";

    let workspaceName = "Notion Workspace";
    try {
        const me = await client.users.me({});
        workspaceName = (isFullUser(me) ? me.name?.trim() : undefined) || workspaceName;
    } catch (e) {
